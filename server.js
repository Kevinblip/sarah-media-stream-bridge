...
async function base44ServiceCall(method, entity, params) {
  const baseUrl = `https://${BASE44_APP_ID}.base44.app`;
  const url = `${baseUrl}/api/entities/${entity}/${method}`;
  const resp = await fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `ServiceRole ${BASE44_SERVICE_ROLE_KEY}`,
    },
    body: JSON.stringify(params),
  });
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`Base44 ${entity}.${method} failed: ${resp.status} ${text}`);
  }
  return resp.json();
}
async function base44Filter(entity, filter) {
  return base44ServiceCall("filter", entity, filter);
}
async function base44Create(entity, data) {
  return base44ServiceCall("create", entity, data);
}
async function base44Update(entity, id, data) {
  const baseUrl = `https://${BASE44_APP_ID}.base44.app`;
  const url = `${baseUrl}/api/entities/${entity}/${id}`;
  const resp = await fetch(url, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`Base44 ${entity}.update failed: ${resp.status} ${text}`);
  }
  return resp.json();
}
async function base44List(entity, sort, limit) {
  const baseUrl = `https://${BASE44_APP_ID}.base44.app`;
  const url = `${baseUrl}/api/entities/${entity}?sort=${encodeURIComponent(sort)}&limit=${limit}`;
  const resp = await fetch(url, {
    method: "GET",
    headers: {
      Authorization: `ServiceRole ${BASE44_SERVICE_ROLE_KEY}`,
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`Base44 ${entity}.list failed: ${resp.status} ${text}`);
  }
  return resp.json();
}
async function getGoogleCalendarToken() {
  if (!BASE44_APP_ID || !BASE44_SERVICE_ROLE_KEY) return null;
  try {
    const baseUrl = `https://${BASE44_APP_ID}.base44.app`;
    const resp = await fetch(`${baseUrl}/api/connectors/googlecalendar/access_token`, {
      headers: { Authorization: `ServiceRole ${BASE44_SERVICE_ROLE_KEY}` },
    });
    if (!resp.ok) return null;
    const data = await resp.json();
    return data.access_token || null;
  } catch (e) {
    console.warn("[GCAL] Failed to get access token:", e.message);
    return null;
  }
}
async function handleCheckAvailability(args, companyId) {
  console.log("[TOOL] Checking availability:", args);
  let accessToken = null;
  try {
    accessToken = await getGoogleCalendarToken();
  } catch (e) {
    console.warn("[GCAL] Token fetch failed:", e.message);
  }
  if (!accessToken) {
    return {
      available_slots: [
  ...
  try {
    ...
    console.error("[GCAL] Availability check failed:", e);
    return { error: "Could not check live calendar. Please propose a time to the user." };
  }
}
async function handleBookAppointment(args, companyId) {
  ...
  try {
    const event = await base44Create("CalendarEvent", {
      company_id: companyId,
      title: `Appt: ${name}`,
      description: description || "Booked via Sarah Voice",
    ...
    try {
      ...
      console.warn("[GCAL] Google Calendar sync failed:", e.message);
    }
    return {
      success: true,
    ...
    console.error("[TOOL] Booking failed:", e);
    return { error: "Failed to book appointment. Please try again." };
  }
}
async function handleSaveLead(args, companyId) {
  console.log("Saving lead:", args);
  try {
    const { name, email, phone, service_needed, address } = args;
    const leads = await base44Filter("Lead", { company_id: companyId });
    let lead = null;
    ...
    if (lead) {
      ...
      if (Object.keys(updates).length > 0) {
        await base44Update("Lead", lead.id, updates);
      }
      return { success: true, message: "Lead updated.", lead_id: lead.id };
    } else {
      const newLead = await base44Create("Lead", {
        company_id: companyId,
        name: name || "Voice Lead",
        email: email,
        phone: phone,
        source: "ai",
        notes: `Captured via Sarah Voice.\nService: ${service_needed || "N/A"}\nAddress: ${address || "N/A"}`,
      });
...
wss.on("connection", async (twilioWs, req) => {
  ...
  console.log(`[CALL] New Twilio stream connection, companyId=${companyId}, scenario=${scenario}`);
  let geminiWs = null;
  let streamSid = null;
  try {
    let settings = {};
    let companyName = "CompanySync";
    if (companyId && BASE44_APP_ID && BASE44_SERVICE_ROLE_KEY) {
      try {
        const settingsRows = await base44Filter("AssistantSettings", {
          company_id: companyId,
          assistant_name: "sarah",
        });
        settings = settingsRows[0] || {};
        const companies = await base44Filter("Company", { id: companyId });
        companyName =
          settings.brand_short_name ||
          companies[0]?.company_name ||
          "our company";
      } catch (e) {
        console.error("[CALL] Failed to load Base44 settings:", e.message);
      }
    }
    const kbParts = [];
              ...
              function_declarations: [
                ...
                },
                {
                  ...
                  parameters: {
                    type: "OBJECT",
                    properties: {
                      slot_time: { type: "STRING", description: "ISO timestamp of the slot" },
    ...
    geminiWs.on("message", async (rawData) => {
      try {
        ...
        if (data.toolCall) {
          ...
          if (useTypingSound && streamSid && twilioWs.readyState === WebSocket.OPEN) {
            twilioWs.send(
              JSON.stringify({
                event: "media",
                streamSid: streamSid,
          ...
          for (const call of data.toolCall.functionCalls) {
            ...
            try {
              if (call.name === "check_availability") {
                result = await handleCheckAvailability(call.args, companyId);
              } else if (call.name === "book_appointment") {
                result = await handleBookAppointment(call.args, companyId);
              } else if (call.name === "save_lead_details") {
                result = await handleSaveLead(call.args, companyId);
              } else {
...
server.listen(PORT, () => {
  ...
  console.log(`WebSocket endpoint: ws://0.0.0.0:${PORT}/ws/twilio`);
